...
+ **从当前公网IP到目标公网IP的路由链路可能非常长，是怎么搜索到链路的？**

  没有找到详细的原理说明，只是看到OSPF使用Dijkstra算法来计算最短路径树，BGP使用路径矢量算法来考虑路径的最优性；但是应该不存在一个路由表能够包含整个互联网公网的路由信息供直接跑算法得到路由链路。

  个人猜测可能公网被拆分成了一些"块"，比如类似自治系统AS，块与块之间可能有**固定的入口和出口IP**，先确定目标公网IP所在的块，找到连接到目标块的最短块路径，然后再在各个块中跑路由算法找到每个块中的路由信息。比如从深圳IP访问北京的IP，先搜索块（假设每个省是一个块），可能找到：广东 -> 湖南 -> 湖北 -> 河南 -> 河北，然后每一块再细化搜索各自的最短路由，比如 广东（深圳 -> 惠州 -> 韶关 )  -> 湖南（衡阳 -> 长沙 -> 岳阳）-> ... -> 河北 (石家庄->北京)。
